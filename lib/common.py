...
from conf.conf import UNICODE_ENCODING
from conf.conf import INVALID_UNICODE_CHAR_FORMAT
from conf.conf import BANNER_1
...
def inputMsg():
    ...
    isIP = False
    plist = []
    msg = "Present by zZt 2015"
    msg += BANNER_1[random.randint(1,5)]
    '''
    ...
    '''
    msg += "\n[1]Now, Input your target:\nFile:\n   /user/local/domain.txt\nDomain:\n   www.baidu.com or baidu.com(subbrute.py)\nIP:\n   192.168.1.1\n   192.168.1.1-10\n   192.168.1.1,192.168.1.2\n   192.168.1.0/24"
    msg += "\ndefault (192.168.1.1)"
    target = readInput(msg,"192.168.1.1")

    msg = "[2]Please enter PORT: [23 or 80,90 or 1-1024]\ndefault (80)"
    port = readInput(msg,80)

    msg = "[3]Please enter thread number:\ndefault (5)"
    num = readInput(msg,5)

    #先处理port，因为后面域名的需要
    if ',' in port:
        plist = port.split(',')#21,22,23
    elif '-' in port:
        part = port.split('-')#1-1024
        for p in range(int(part[0]),int(part[1])+1):
            plist.append(p)
    else:
        plist = [int(port)]#80
    ...
    if valicateIp(target):
        #处理ip地址
        isIP = True
        if ',' in target:#192.168.1.1,192.168.1.2
            tlist = target.split(',')
        elif '-' in target:#192.168.1.1-9
            part = target.split('-')
            part1 = part[0].split('.')
            for i in range(int(part1[3]),int(part[1])+1):
                tlist.append(part1[0]+'.'+part1[1]+'.'+part1[2]+'.'+str(i))
        else:
            tlist.append(target)#192.168.1.1
        ...
        tlist = [ _.strip() for _ in open(target)]
    else:
        print "[!]Error input IP is wrong\n"
        sys.exit(1)
